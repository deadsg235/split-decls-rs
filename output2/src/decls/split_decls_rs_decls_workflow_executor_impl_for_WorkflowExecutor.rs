// Generated by add_generated_rust_header! macro at submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs:71 from submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs at 2025-12-23T17:36:50.128326392+00:00
...
impl WorkflowExecutor {
    ...
    fn execute_stage(&mut self, stage: &Stage) -> Result<()> {
        ...
        match &stage.operation {
            Operation::FunctionCall(op) => {
                self.execute_function_call(op, &stage.inputs, &stage.outputs)?
            }
            Operation::Loop(op) => {
                let collection_toml_value = self
                    .context
                    .get(&op.over)
                    .context(format!("Loop variable '{}' not found in context", op.over))?;
                let collection_to_loop: Vec<toml::Value> = collection_toml_value
                    .as_array()
                    .context(format!("Loop variable '{}' is not an array", op.over))?
                    .clone();
                for item_toml_value in collection_to_loop {
                    self.context
                        .insert(op.loop_var.clone(), item_toml_value.clone());
                    if self.verbose {
                        println!(
                            "    Loop iteration: {} = {:?}",
                            op.loop_var, item_toml_value
                        );
                    }
            ...
            Operation::Shell(op) => {
                self.execute_shell_command(op, &stage.inputs, &stage.outputs)?
            }
            Operation::Unknown(value) => {
                anyhow::bail!(
                    "Unknown operation type encountered in stage {}: {:?}",
                    stage.name,
                    value
                );
            }
        ...
        if let Some(accumulator) = processed_crates_accumulator {
            if stage.name == "Extract and Copy Declarations" {
                self.context.insert(
                    "all_processed_crates_info".to_string(),
                    toml::Value::Array(accumulator),
                );
                if self.verbose {
                    println!(
                        "  Aggregated {} processed crate infos as 'all_processed_crates_info'.",
                        self.context["all_processed_crates_info"]
                            .as_array()
    ...
    fn execute_function_call(
        ...
        match op.function.as_str() {
            "find_crates" => {
                ...
                let base_path_str = resolved_args
                    .get(0)
                    .context("find_crates expects a base_path argument")?;
                        ...
                        discovered_crates
                            ...
                            .map(|ci| {
                                ...
                                table.insert(
                                    ...
                                    toml::Value::String(
                                        ci.original_path.to_string_lossy().into_owned(),
                                    ),
                                );
            ...
            "extract_declarations" => {
                ...
                let crate_path_str = resolved_args
                    .get(0)
                    .context("extract_declarations expects a crate_path argument")?;
            ...
            "copy_to_output" => {
                ...
                let crate_name = resolved_args
                    .get(0)
                    .context("copy_to_output expects crate_name as the first argument")?;
                let declarations_context_key = resolved_args.get(1).context(
                    "copy_to_output expects declarations context key as the second argument",
                )?;
                let output_base_path_str = resolved_args
                    .get(2)
                    .context("copy_to_output expects output_base_path as the third argument")?;
                let output_base_path = PathBuf::from(output_base_path_str);
                let declarations_toml = self
                    .context
                    .get(declarations_context_key)
                    .context(format!(
                        "Declarations '{}' not found in context for copy_to_output",
                        declarations_context_key
                    ))?
                    .as_table()
                    .context(format!(
                        "Declarations '{}' in context is not a table for copy_to_output",
                        declarations_context_key
                    ))?;
                let mut declarations_map: HashMap<String, String> = HashMap::new();
                for (name, value) in declarations_toml {
                    declarations_map.insert(
                        name.clone(),
                        value
                            .as_str()
                            .context("Declaration value is not a string")?
                            .to_string(),
                    );
                }
                eager_splitter::copy_declarations_to_output(
                    crate_name,
                    &declarations_map,
                    &output_base_path,
                )?;
                if self.verbose {
                    println!(
                        "  Copied {} declarations for crate '{}' to {}",
                        declarations_map.len(),
                        crate_name,
                        output_base_path.display()
                    );
                }
            }
            "generate_workspace_root_toml" => {
                ...
                let output_dir_str = resolved_args.get(0).context(
                    "generate_workspace_root_toml expects output_dir as the first argument",
                )?;
                ...
                let processed_crates_toml = self . context . get (processed_crates_context_key) . context (format ! ("Processed crates '{}' not found in context for generate_workspace_root_toml" , processed_crates_context_key)) ? . as_array () . context (format ! ("Processed crates '{}' in context is not an array for generate_workspace_root_toml" , processed_crates_context_key)) ? ;
                let processed_crate_paths: Vec<PathBuf> = processed_crates_toml
                    .iter()
                    .filter_map(|v| v.as_str())
                    .map(PathBuf::from)
                    .collect();
                generate_new_workspace::generate_root_toml(
                    &output_dir,
                    &processed_crate_paths,
                    self.dry_run,
                )?;
                if self.verbose {
                    println!(
                        "  Generated workspace root Cargo.toml in {}",
                        output_dir.display()
                    );
                }
            }
            "generate_crate_toml" => {
                ...
                let output_crate_dir_str = resolved_args.get(0).context(
                    "generate_crate_toml expects output_crate_dir as the first argument",
                )?;
                let crate_info_context_key = resolved_args.get(1).context(
                    "generate_crate_toml expects crate_info context key as the second argument",
                )?;
                let output_crate_dir = PathBuf::from(output_crate_dir_str);
                let crate_info_toml = self
                    .context
                    .get(crate_info_context_key)
                    .context(format!(
                        "Crate info '{}' not found in context for generate_crate_toml",
                        crate_info_context_key
                    ))?
                    .as_table()
                    .context(format!(
                        "Crate info '{}' in context is not a table for generate_crate_toml",
                        crate_info_context_key
                    ))?;
                let crate_name = crate_info_toml
                    .get("name")
                    .context("Crate info missing 'name' field")?
                    .as_str()
                    .context("Crate name is not a string")?
                    .to_string();
                let original_crate_path_str = crate_info_toml
                    .get("original_path")
                ...
                if self.verbose {
                    println!(
                        "  Generated Cargo.toml for crate '{}' in {}",
                        crate_name,
    ...
    fn execute_shell_command(
        ...
        let program = command_parts
            .next()
            .context("Shell command cannot be empty")?;
        ...
        if let Some(ref wd_str) = op.working_dir {
            ...
            if !work_dir.exists() {
                fs::create_dir_all(&work_dir).context(format!(
        ...
        if self.dry_run {
            ...
            if let Some(output_def) = outputs.iter().find(|o| o.name == "stdout") {
                self.context.insert(
                    ...
                    toml::Value::String("(dry run stdout)".to_string()),
                );
            }
            if let Some(output_def) = outputs.iter().find(|o| o.name == "stderr") {
                self.context.insert(
                    output_def.name.clone(),
                    toml::Value::String("(dry run stderr)".to_string()),
                );
            }
            if let Some(output_def) = outputs.iter().find(|o| o.name == "status") {
                self.context
                    .insert(output_def.name.clone(), toml::Value::Integer(0));
            }
            return Ok(());
        }
        let output = command
        ...
        if op.capture_output {
            if let Some(output_def) = outputs.iter().find(|o| o.name == "stdout") {
                self.context
                    .insert(output_def.name.clone(), toml::Value::String(stdout.clone()));
            }
            if let Some(output_def) = outputs.iter().find(|o| o.name == "stderr") {
                self.context
                    .insert(output_def.name.clone(), toml::Value::String(stderr.clone()));
            }
            if let Some(output_def) = outputs.iter().find(|o| o.name == "status") {
                self.context
                    .insert(output_def.name.clone(), toml::Value::Integer(status as i64));
            }
        }
    ...
    fn execute_switch(
        ...
        let value_to_match = self.context.get(&op.match_on).context(format!(
            "Match variable '{}' not found in context for switch operation",
            op.match_on
        ))?;
        let mut matched = false;
        for (case_value_str, tasks) in &op.cases {
            let context_value_str = value_to_match.as_str().context(format!(
                "Context value for '{}' is not a string, cannot match against cases",
                op.match_on
            ))?;
    ...
    fn execute_task(
        ...
        task: &Task,
        parent_inputs: &[Input],
        parent_outputs: &[Output],
    ) -> Result<()> {
        if self.verbose {
            println!("    Executing task: {}", task.name);
        }
        let task_inputs = if task.inputs.is_empty() {
            parent_inputs
        } else {
            &task.inputs
        };
        let task_outputs = if task.outputs.is_empty() {
            parent_outputs
        } else {
            &task.outputs
        };
        match &task.operation {
            Operation::FunctionCall(op) => {
                self.execute_function_call(op, task_inputs, task_outputs)?
            }
            ...
            Operation::Shell(op) => self.execute_shell_command(op, task_inputs, task_outputs)?,
            Operation::Unknown(value) => {
                anyhow::bail!(
                    "Unknown operation type encountered in task {}: {:?}",
                    task.name,
                    value
                );
            }
        ...
        if arg.starts_with("{{") && arg.ends_with("}}") {
            ...
            if let Some(value) = self.context.get(path) {
                if value.is_str() {
                    ...
                    anyhow::bail!(
                        "Unsupported context value type or complex path for arg: {}",
                        arg
                    );
                }
            } else {
