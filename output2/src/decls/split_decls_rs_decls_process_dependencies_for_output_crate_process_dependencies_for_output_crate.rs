                ...
                && dep_table["workspace"].as_bool().unwrap_or(false)
            {
                if let Some(resolved_dep_info) = global_config.workspace_dependencies.get(dep_name)
                {
                    if let Some(resolved_path_str) =
                        resolved_dep_info.get("path").and_then(|v| v.as_str())
                    {
                        ...
                        processed_dep_table.insert(
                            "path".to_string(),
                            Value::String(relative_path.display().to_string()),
                        );
                        processed_dep_table.remove("workspace");
                        processed_dep_table.remove("version");
                    } else if let Some(resolved_version_str) =
                        resolved_dep_info.get("version").and_then(|v| v.as_str())
                    {
                        processed_dep_table.insert(
                            "version".to_string(),
                            Value::String(resolved_version_str.to_string()),
                        );
                        processed_dep_table.remove("workspace");
                    } else {
                        if verbose {
                ...
                if let Some(original_path_str) = original_path_value.as_str() {
                    ...
                    let relative_path =
                        path_diff(output_dir, &absolute_original_path).context(format!(
                            "Failed to calculate relative path for path dep '{}'",
                            dep_name
                        ))?;
                    processed_dep_table.insert(
                        "path".to_string(),
