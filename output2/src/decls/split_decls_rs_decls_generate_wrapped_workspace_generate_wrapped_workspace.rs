// Generated by add_generated_rust_header! macro at submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs:71 from submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs at 2025-12-23T17:27:47.211426646+00:00
...
pub fn generate_wrapped_workspace(
    ...
    if patch_config.generated_workspace_member.is_empty() && main_crate_cargo_toml_path.exists() {
        ...
        if let Some(deps) = root_cargo_toml
            .get("dependencies")
            .and_then(|v| v.as_table())
        {
            package_deps.extend(deps.clone());
        }
        if let Some(build_deps) = root_cargo_toml
            .get("build-dependencies")
            .and_then(|v| v.as_table())
        {
            package_build_deps.extend(build_deps.clone());
        }
        if let Some(dev_deps) = root_cargo_toml
            ...
            if let Some(dep_table) = processed_dep_value.as_table_mut() {
                if let Some(path_value) = dep_table.get("path") {
                    if let Some(path_str) = path_value.as_str() {
                        ...
                        let relative_path =
                            path_diff(output_dir, &absolute_dep_path).context(format!(
                                "Failed to calculate relative path for workspace dependency '{}'",
                                dep_name
                            ))?;
                        dep_table.insert(
                            "path".to_string(),
