// Generated by add_generated_rust_header! macro at submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs:71 from submodules/split-decls-rs/src/eager_splitter/declaration_writer.rs at 2025-12-23T17:36:48.427693930+00:00
...
fn main() -> Result<()> {
    ...
    let eigenvalue_threshold = total_macros / 100;
    let core_macros: Vec<_> = sorted_macros
        .iter()
        .filter(|(_, count)| **count >= eigenvalue_threshold)
        .map(|(name, count)| (name.to_string(), **count))
        .collect();
    println!(
        "\n🎯 Core Eigenmatrix ({} fundamental macros):",
        core_macros.len()
    );
    for (name, count) in &core_macros {
        println!("  {} -> {}", name, count);
    }
    let eigenmatrix_data = serde_json :: json ! ({ "total_macros" : total_macros , "unique_patterns" : macro_frequency . len () , "core_macros" : core_macros , "frequency_distribution" : macro_frequency });
    fs::write(
        "rust_eigenmatrix.json",
        serde_json::to_string_pretty(&eigenmatrix_data)?,
    )?;
