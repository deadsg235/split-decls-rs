...
// Import the external modules created
use crate::buildrs_config::{PatchSpecBuildrs, StringReplacementBuildrs, SplitDeclsConfigBuildrs};
use crate::buildrs_ast_utils::{ExtractedDecl, UseStatementCollector};
...
pub fn generate_build_rs_token_stream(
    ...
    let config_types_ts = generate_build_rs_config_types_for_generated_buildrs();
    let macros_ts = generate_build_rs_macros();
    let ast_helpers_ts = generate_build_rs_ast_helpers_for_generated_buildrs();
...
// NOTE: These helper functions will be moved into their own files.
// For now, they are temporarily here to compile.
...
fn generate_build_rs_config_types_for_generated_buildrs() -> TokenStream {
    quote! {
...
fn generate_build_rs_ast_helpers_for_generated_buildrs() -> TokenStream {
    quote! {
