...
fn setup_crate_paths(crate_path: &Path) -> Result<CratePaths> {
...
fn handle_git_overlay(repo_url: &str, target_dir: &Path, reference: &str) -> Result<()> {
...
fn process_crate(crate_path: &Path, global_config: &SplitDeclsConfig) -> Result<()> {
...
fn main() -> Result<()> {
    ...
    process_crates_in_path(&workspace_root, current_crate_name, &global_config, false)?;
    ...
    if let Some(rustc_source_path_val) = &global_config.rustc_source_path {
        ...
        handle_git_overlay(rustc_repo_url, rustc_source_path_val, &rustc_reference)?;

        // Now iterate over the crates within the rustc source tree, with is_rustc_source = true
        process_crates_in_path(rustc_source_path_val, current_crate_name, &global_config, true)?;
    }
